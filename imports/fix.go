...
// Modified to respect a .goignore file in $GOPATH. The
// .goignore file is a line-by-line list of directories
...
func loadGoIgnores() (ignores []string) {
...
func loadPkg(wg *sync.WaitGroup, root, pkgrelpath string) {
	...
	for _, child := range children {
		...
		// Respect .goignore.
		ignores := loadGoIgnores()
		skip := false
		fullpath := filepath.Join(importpath, name)
		for _, ignore := range ignores {
			if child.IsDir() && strings.Contains(fullpath, ignore) {
				skip = true
				break
			}
		}
